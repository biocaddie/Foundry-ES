...
public class HtmlEntityUtil {
    private static final String HTML_ENTITIES =
            "nbsp   #160 00A0   iexcl  #161 00A1   cent   #162 00A2   pound  #163 00A3   curren #164 00A4   yen    #165 00A5 " +
                    ...
                    "euro    #8364 20AC  quote #034 0022 backslash #092 005C";
    static Map<String, Character> he2charMap = new HashMap<String, Character>();

    static {
        String[] toks = HTML_ENTITIES.split("\\s+");
        for(int i = 0; i < toks.length; i+= 3) {
            char c = (char) Integer.parseInt(toks[i + 2], 16);
            he2charMap.put(toks[i], c);
            he2charMap.put(toks[i+1], c);
        }
    }

    public static Character getChar(String htmlEntity) {
        if (htmlEntity.startsWith("&")) {
            htmlEntity = htmlEntity.substring(1);
        }
        int idx = htmlEntity.indexOf(';');
        if (idx != -1) {
            htmlEntity = htmlEntity.substring(0, idx);
        }
        return he2charMap.get(htmlEntity);
    }

    public static boolean isHtmlEntity(String s) {
        return s.startsWith("&#");
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("s:" + s + " c:" + HtmlEntityUtil.getChar(s));
        s = "&#092;";
        System.out.println("s:" + s + " c:" + HtmlEntityUtil.getChar(s));
    }
}
